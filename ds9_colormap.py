...
class ds9_colormap:
    def __init__(self,CIAO='',ds9_cmap=''):
        ...
        if os.path.isfile(ds9_cmap_loc):
            ...
            print("you can use it directly as: plt.imshow('myimage.fits',cmap='%s',norm=LogNorm(),vmin=7e-7,vmax=1e-3,origin='lower')"%self.ds9_cmap_name)
    ...
    def show(self, image=None, vmin=None, vmax=None, log=True, cmap=None, with_cmap=False, dpi=100, file_reg=None, pan_to=None, zoom=None, show_axes=False):
        if image is None:
            image='example.fits'
        if cmap is None:
        ...
        if pan_to != None:
            ...
            crop_lim=[pan_to[0]-pan_to[2]/2,pan_to[0]+pan_to[2]/2,pan_to[1]-pan_to[3]/2,pan_to[1]+pan_to[3]/2]
        else:
            if zoom != None:
        ...
        fig=plt.figure(figsize=(nx/dpi,ny/dpi),tight_layout=True)
        if with_cmap == True:
